...
< Result 77 >*/
    ...
    double customOperands[] = { 5, 16, -3, 10, 12 };
    CustomExpressionEvaluator* customExpressionEvaluator = new CustomExpressionEvaluator(5);
    evaluators[1] = customExpressionEvaluator;
    evaluators[1]->setOperands(customOperands, 5);
    ...
    {
        evaluators[i]->logToFile("D:\\Lab3(1).log");
        evaluators[i]->logToScreen();
        std::cout <<" < Total "<< evaluators[i]->getN() << " >" << std::endl;
    ...
    for (size_t i = 0;i < 3;++i) 
    {
        ...
        {
            std::cout << std::endl;
            std::cout <<i+1<< " is shuffleable!" << std::endl;
